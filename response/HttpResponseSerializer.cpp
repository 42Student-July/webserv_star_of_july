...
HttpResponsePlainText *HttpResponseSerializer::serialize(HttpResponse &res)
{
	...
	ss << PROTOCOL << res.Version() << SP << res.StatusCode() << SP << res.ReasonPhrase() << CRLF
	   << SERVER << res.Server() << CRLF
	   << DATE << res.Date() << CRLF
	   << CONTENT_TYPE << res.ContentType() << CRLF
	   << CONTENT_LENGTH << res.ContentLength() << CRLF
	   << LAST_MODIFIED << res.LastModified() << CRLF
	   << CONNECTION << res.Connection() << CRLF
	   << ETAG << res.ETag() << CRLF
	   << ACCEPT_RANGES << res.AcceptRanges() << CRLF
	   << CRLF
	   << res.Body() << CRLF
	   << CRLF;
